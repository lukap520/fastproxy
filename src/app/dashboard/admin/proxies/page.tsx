            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.32 }} style={{ marginBottom: 32, display: "flex", flexDirection: isSmall ? "column" : "row", alignItems: isSmall ? "flex-start" : "flex-end", gap: isSmall ? 20 : 0, justifyContent: "space-between" }}>
                <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 12 }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,107,0,0.95)", fontFamily: "var(--font-sans,system-ui)", letterSpacing: "0.15em", textTransform: "uppercase", textShadow: "0 1px 2px rgba(0,0,0,0.2)" }}>Administration</span>
                    </div>
                    <h1 style={{ fontFamily: "var(--font-heading,'Clash Display',system-ui)", fontSize: "2.2rem", fontWeight: 700, color: "#FFFFFF", letterSpacing: "-0.04em", textShadow: "0 2px 4px rgba(0,0,0,0.3)" }}>
                        Active Proxies
                    </h1>
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <AnimatePresence mode="wait">
                    {filteredProxies?.length === 0 ? (
                        <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ padding: "60px 20px", textAlign: "center", color: "rgba(255,255,255,0.3)", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 16 }}>
                            <Icon icon="ph:ghost" style={{ fontSize: 40, marginBottom: 10, opacity: 0.5 }} />
                            <p>No active proxies found matching your criteria.</p>
                        </motion.div>
                    ) : (
                        <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: "grid", gridTemplateColumns: isSmall ? "1fr" : "repeat(2, 1fr)", gap: 16 }}>
                                        ...
                                        <GlassCard style={{ padding: 20 }}>
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
                                                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                                                    ...
                                                    <div>
                                                        <h3 style={{ fontSize: 14.5, fontWeight: 700, color: "#FFFFFF", margin: 0, textShadow: "0 1px 2px rgba(0,0,0,0.2)" }}>Residential Proxy</h3>
                                            ...
                                            <div>
                                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                                                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", textShadow: "0 1px 2px rgba(0,0,0,0.2)" }}>Data Usage</span>
                                                    <span style={{ fontSize: 12.5, color: "#FFFFFF", fontWeight: 700, textShadow: "0 1px 2px rgba(0,0,0,0.2)" }}>{p.gbUsed.toFixed(2)} / {p.allocatedGb} GB</span>
                                                </div>
