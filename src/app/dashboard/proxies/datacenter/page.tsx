...
export default function DatacenterPage() {
    ...
    const { data: plans, isLoading: plansLoading, refetch } = trpc.flashproxy.listPlans.useQuery({ product: "datacenter", status: "active" });
    ...
    const create = trpc.flashproxy.createPlan.useMutation({
        onSuccess: () => { toast("success", "Datacenter plan created!"); refetch(); },
    ...
    const pricePerGb = pricing?.["datacenter"]?.price_per_gb_cents ? pricing["datacenter"].price_per_gb_cents / 100 : 0.30;
    ...
    const generateProxies = () => {
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            if (proxyType === "sticky") u += `-session-${Math.random().toString(36).slice(2, 10)}`;
        ...
        <div style={{ flex: 1, padding: isSmall ? "80px 20px 40px" : "52px 56px", maxWidth: 1060, width: "100%", margin: "0 auto" }}>
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.32 }} style={{ marginBottom: 44 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 12 }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase" as const, color: "rgba(255,107,0,0.9)", fontFamily: "var(--font-sans,system-ui)" }}>Datacenter</span>
                </div>
                <h1 style={{ fontFamily: "var(--font-heading,system-ui)", fontSize: "2rem", fontWeight: 700, color: "#FFFFFF", letterSpacing: "-0.04em", lineHeight: 1.1, marginBottom: 6 }}>Datacenter Proxies</h1>
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", fontFamily: "var(--font-sans,system-ui)" }}>
                    High-speed. <span style={{ color: "#FFFFFF", fontWeight: 600 }}>Per-GB or time-based</span> billing · Gbps speeds · HTTP &amp; SOCKS5
                </p>
            </motion.div>
            ...
            {activePlan ? (
                <div style={{ display: "grid", gridTemplateColumns: isSmall ? "1fr" : "1fr 300px", gap: 18, alignItems: "start" }}>
                        ...
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }}
                            ...
                            <div style={{ padding: "18px 20px", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                    ...
                                    <span style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", fontFamily: "var(--font-sans,system-ui)" }}>Active Plan · {activePlan.billing_type === "bandwidth" ? "Bandwidth" : "Time-based"}</span>
                                </div>
                                ...
                                {activePlan.billing_type === "bandwidth" ? (
                                    ...
                                    <div>
                                        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", fontFamily: "var(--font-sans,system-ui)" }}>
                                            {activePlan.limits.bandwidth_mbps ? `${activePlan.limits.bandwidth_mbps} Mbps` : "Unlimited bandwidth"}
                                        </p>
                                        {activePlan.expires_at && (
                                            <p style={{ marginTop: 6, fontSize: 11, color: "rgba(255,255,255,0.3)", fontFamily: "var(--font-sans,system-ui)" }}>
                                    ...
                                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} style={{ overflow: "hidden", marginTop: 12 }}>
                                        {activePlan.billing_type === "bandwidth" ? (
                                                    ...
                                                    {[5, 10, 25, 50, 100, 200].map(v => {
                                                        ...
                                                        return (
                                                            <button key={v} type="button" onClick={() => setAddGb(v)}
                                                                style={{ padding: "9px 6px", borderRadius: 9, border: on ? "1px solid rgba(255,107,0,0.4)" : "1px solid rgba(255,255,255,0.06)", background: on ? "rgba(255,107,0,0.08)" : "rgba(255,255,255,0.018)", color: on ? "rgb(255,107,0)" : "rgba(255,255,255,0.4)", fontSize: 12, fontWeight: 700, cursor: "pointer", transition: "all 0.13s" }}>
                                                                +{v} GB
                                                            </button>
                                                        );
                                                    })}
                                            ...
                                            <>
                                                ...
                                                <button type="button" disabled={extend.isPending}
                                                    onClick={() => extend.mutate({ planId: activePlan.plan_id, add_days: addDays })}
                                                    style={{ width: "100%", padding: "10px", borderRadius: 10, border: "none", background: "hsl(24, 100%, 45%)", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "var(--font-sans,system-ui)", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
                    ...
                    <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.08 }}
                        ...
                        <div style={{ padding: "16px 20px" }}>
                            {[
                                ["Plan ID", activePlan.plan_id.slice(-12).toUpperCase()],
                                ["Type", activePlan.billing_type === "bandwidth" ? "Bandwidth" : "Time-based"],
                                ...(activePlan.expires_at ? [["Expires", new Date(activePlan.expires_at).toLocaleDateString()]] : []),
                                ["Cost", activePlan.billing?.cost_formatted ?? "—"],
                            ].map(([k, v]) => (
                ...
                <div style={{ display: "grid", gridTemplateColumns: isSmall ? "1fr" : "1fr 440px", gap: 20, alignItems: "start" }}>
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }}>
                        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.11em", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-sans,system-ui)", marginBottom: 14 }}>Why Datacenter?</p>
                        {[
                            { icon: "ph:lightning-fill", label: "Gbps speeds", sub: "Lightning-fast datacenter IPs. Ideal for tasks requiring maximum speed." },
                            { icon: "ph:timer", label: "Flexible billing", sub: "Pay per GB used or rent by time with a defined Mbps cap." },
                            { icon: "ph:arrows-out", label: "Scalable", sub: "Generate thousands of proxy strings from a single plan." },
                            { icon: "ph:shield-check", label: "HTTP & SOCKS5", sub: "Compatible with all tools and browsers." },
                        ].map((f, i) => (
                    ...
                    <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
                        ...
                        <div style={{ padding: "22px", display: "flex", flexDirection: "column", gap: 18 }}>
                            <div>
                                <p style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.7)", marginBottom: 8, fontFamily: "var(--font-sans,system-ui)" }}>Billing Type</p>
                                <div style={{ display: "flex", background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: 4 }}>
                                    {(["bandwidth", "time"] as const).map(t => (
                                        <button key={t} type="button" onClick={() => setBillingType(t)}
                                            style={{ flex: 1, padding: "8px", borderRadius: 8, border: "none", background: billingType === t ? "rgba(255,255,255,0.08)" : "transparent", color: billingType === t ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.4)", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all 0.2s", textTransform: "capitalize" as const }}>
                            ...
                            {billingType === "bandwidth" ? (
                                <div>
                                    <p style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.7)", marginBottom: 8, fontFamily: "var(--font-sans,system-ui)" }}>Bandwidth</p>
                                            ...
                                            {DURATIONS.map(d => {
                                                ...
                                                return (
                                                    <button key={d.id} type="button" onClick={() => setDuration(d.id as "1_hour" | "1_day" | "7_days" | "30_days")}
                                                        style={{ padding: "9px", borderRadius: 9, border: on ? "1px solid rgba(255,107,0,0.4)" : "1px solid rgba(255,255,255,0.06)", background: on ? "rgba(255,107,0,0.08)" : "rgba(255,255,255,0.018)", color: on ? "rgb(255,107,0)" : "rgba(255,255,255,0.7)", fontSize: 12, fontWeight: 700, cursor: "pointer", transition: "all 0.13s" }}>
                                                        {d.label}
                                                    </button>
                                                );
                                            })}
                            ...
                            {billingType === "time" && (
                                <p style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", fontFamily: "var(--font-sans,system-ui)" }}>
                                    Exact price calculated based on duration and speed.
                                </p>
                            )}
                            ...
                            <button type="button" disabled={create.isPending || (billingType === "bandwidth" && !canAfford)}
                                onClick={() => create.mutate({
                                    ...
                                    ...(billingType === "bandwidth" ? { bandwidth_gb: gb } : { duration, mbps }),
                                })}
