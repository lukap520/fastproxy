...
export default function IPv6DatacenterPage() {
    ...
    const create = trpc.flashproxy.createPlan.useMutation({
        onSuccess: () => { toast("success", "IPv6 Datacenter plan created!"); refetch(); },
        ...
        <div style={{ flex: 1, padding: isSmall ? "80px 20px 40px" : "52px 56px", maxWidth: 1060, width: "100%", margin: "0 auto" }}>
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.32 }} style={{ marginBottom: 44 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 12 }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase" as const, color: "rgba(255,107,0,0.9)", fontFamily: "var(--font-sans,system-ui)" }}>IPv6 Datacenter</span>
                </div>
                <h1 style={{ fontFamily: "var(--font-heading,system-ui)", fontSize: "2rem", fontWeight: 700, color: "#FFFFFF", letterSpacing: "-0.04em", lineHeight: 1.1, marginBottom: 6 }}>IPv6 Datacenter Proxies</h1>
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", fontFamily: "var(--font-sans,system-ui)" }}>
                    Full IPv6 support. <span style={{ color: "#FFFFFF", fontWeight: 600 }}>Per-GB or time-based</span> · Gbps speeds · HTTP &amp; SOCKS5
                </p>
            </motion.div>
            ...
            {activePlan ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: isSmall ? "1fr" : "1fr 440px", gap: 20, alignItems: "start" }}>
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }}>
                        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.11em", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-sans,system-ui)", marginBottom: 14 }}>Why IPv6 Datacenter?</p>
                        {[
                            { icon: "ph:cpu", label: "IPv6 native", sub: "Full IPv6 protocol support from datacenter infrastructure. Ideal for IPv6-only APIs and targets." },
                            { icon: "ph:lightning-fill", label: "Gbps speeds", sub: "Datacenter-grade performance with ultra-low latency." },
                            { icon: "ph:timer", label: "Flexible billing", sub: "Pay per GB or rent by hour/day/month. Mbps cap for time-based plans." },
                            { icon: "ph:arrows-out", label: "Scalable usage", sub: "Generate thousands of proxy strings from a single plan." },
                        ].map((f, i) => (
                    ...
                    <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
                        ...
                        <div style={{ padding: "22px", display: "flex", flexDirection: "column", gap: 18 }}>
                            ...
                            <button type="button" disabled={create.isPending || (billingType === "bandwidth" && !canAfford)}
                                onClick={() => create.mutate({ product: "ipv6-datacenter", billing_type: billingType, ...(billingType === "bandwidth" ? { bandwidth_gb: gb } : { duration, mbps }) })}
