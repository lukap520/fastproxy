...
function IPv6Page({ product, title, subtitle, username_format }: { product: "ipv6-residential" | "ipv6-datacenter"; title: string; subtitle: string; username_format: string }) {
    ...
    const create = trpc.flashproxy.createPlan.useMutation({
        onSuccess: () => { toast("success", `${title} plan created!`); refetch(); },
    ...
    const generateProxies = () => {
        ...
        return Array.from({ length: quantity }, (_, i) => {
            let user_str = activePlan.proxy_username;
            if (country !== "any") user_str += `-country-${country}`;
            return `${user_str}-package-${product === "ipv6-residential" ? "ipv6" : "ipv6dc"}:${activePlan.proxy_password}@${activePlan.connection.hostname}:${activePlan.connection.port_http}`;
        }).join("\n");
    };
    ...
    const COUNTRIES_SHORT = ["any", "US", "GB", "DE", "FR", "CA", "AU", "NL", "JP", "BR", "IN"];
        ...
        <div style={{ flex: 1, padding: isSmall ? "80px 20px 40px" : "52px 56px", maxWidth: 1060, width: "100%", margin: "0 auto" }}>
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.32 }} style={{ marginBottom: 44 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 12 }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase" as const, color: "rgba(255,107,0,0.9)", fontFamily: "var(--font-sans,system-ui)" }}>{title}</span>
                </div>
                <h1 style={{ fontFamily: "var(--font-heading,system-ui)", fontSize: "2rem", fontWeight: 700, color: "#FFFFFF", letterSpacing: "-0.04em", lineHeight: 1.1, marginBottom: 6 }}>{title} Proxies</h1>
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", fontFamily: "var(--font-sans,system-ui)" }}>{subtitle}</p>
            </motion.div>
            ...
            {activePlan ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: isSmall ? "1fr" : "1fr 440px", gap: 20, alignItems: "start" }}>
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }}>
                        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.11em", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-sans,system-ui)", marginBottom: 14 }}>Why {title}?</p>
                        {(product === "ipv6-residential" ? [
                            ...
                            { icon: "ph:timer", label: "Flexible billing", sub: "Bandwidth-based or time-based. Rent by hour up to 90 days." },
                            { icon: "ph:arrows-out", label: "Scalable", sub: "Use one plan to generate thousands of proxy strings for your tasks." },
                        ]).map((f, i) => (
                    ...
                    <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
                        ...
                        <div style={{ padding: "22px", display: "flex", flexDirection: "column", gap: 18 }}>
                            ...
                            <button type="button" disabled={create.isPending || (billingType === "bandwidth" && !canAfford)}
                                onClick={() => create.mutate({ product, billing_type: billingType, ...(billingType === "bandwidth" ? { bandwidth_gb: gb } : { duration, mbps }) })}
...
export default function IPv6ResidentialPage() {
    return (
        <IPv6Page
            product="ipv6-residential"
